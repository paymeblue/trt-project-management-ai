...
export default function NewProjectForm() {
  ...
  function onChange(stepN: number, value: string) {
    if (value) {
      ...
      if (min && value < min) {
        const earlier = ACTIONABLE_STEPS.filter((s) => s.n < stepN && deadlines[s.n] === min)[0]
        setToast(
      ...
      if (max && value > max) {
        const later = ACTIONABLE_STEPS.filter((s) => s.n > stepN && deadlines[s.n] === max)[0]
        setToast(
    ...
    <form action={action} className="space-y-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
      ...
      <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
        ...
        <p className="mb-3 text-[11px] text-gray-500">
          ...
          deadline (steps {FIRST_ACTION_STEP}–{LAST_STEP}). A later step can’t be due before an
          earlier one.
        </p>
