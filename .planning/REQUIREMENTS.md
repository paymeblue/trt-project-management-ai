...
- [x] **ROLE-02**: An "assignment" step lets its actor (e.g., Head Designer) pick a user from a target pool that may span more than one role (e.g. `design` and `architect` both), and notifies that user they've been assigned.
  ...
  - *Verified 2026-07-11 by `scripts/verify-role-assignment.ts`:* the pool-membership half is confirmed against the real `assign_designer_brief` live step — a `design`/`architect`-role user is accepted, an out-of-pool `factory_pm` user is rejected (`assignee-role-mismatch`). The assignee-notification gap found the same day (`assignUser`/`assignUserAction` recorded the assignment but never wrote to the `notifications` table) is now closed (quick task 260711-assignee-notification): a single-recipient `notifyUser` (`lib/notifications.ts`) fires from `assignUser` right after the durable write, self-excluding on auto-assign; the header bell (`app/_components/notifications-bell.tsx`) is mounted for every authenticated role (was super-admin-only) so the assignee can actually see it, and `markOne` no longer routes `'assignment'`-typed notifications to the super-admin-only `/disputes` thread. `verify-role-assignment.ts`'s ROLE-02b assertion now requires and confirms a `type === 'assignment'` row.
